...
// Custom REST API for Gravity Forms
add_action('rest_api_init', function () {
    register_rest_route('react-theme/v1', '/form/(?P<id>\d+)', array(
        'methods' => 'GET',
        'callback' => 'react_theme_get_form',
        'permission_callback' => '__return_true', // Open for demo purposes
    ));

    register_rest_route('react-theme/v1', '/form/(?P<id>\d+)/submit', array(
        'methods' => 'POST',
        'callback' => 'react_theme_submit_form',
...
function react_theme_submit_form($request) {
    if (!class_exists('GFAPI')) {
        return new WP_Error('gf_missing', 'Gravity Forms not active', array('status' => 500));
    }
    ...
    // Simplistic submission - in prod, validate nonce/etc
    $result = GFAPI::submit_form($form_id, $params);
